...
| mcp-compressor self-reported percentages | its startup banner | **✗ mismeasured — root-caused from its source** (`banner.rs`): character counts, no tokenizer, and the ratio's two sides count different things; formula replicated in Python, reported upstream as [mcp-compressor#236](https://github.com/atlassian-labs/mcp-compressor/issues/236) ([MCP-COMPRESSOR.md](MCP-COMPRESSOR.md)) |
