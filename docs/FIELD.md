...
| Schema compression proxy | [mcp-compressor](MCP-COMPRESSOR.md) (Atlassian) | 23.7–103.8% self-reported (level/scale-dependent) | **⚠ discrepancy found**: our tokenizer measured **+12% saving** on the same output where its banner claimed a 103.8% *loss*; scale effect confirmed (+67% on a 12-tool server) |
...
| Schemas cost 5–15× a minimal rendering; tiered schemas would save 60–70% | [MCP spec issue #2808](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/2808) | ✅ the ratio matches our data (compact form: +78–89% on real servers/specs); the tiered-proposal simulation is next (roadmap N7) |
| mcp-compressor self-reported percentages | its startup banner | ⚠ disagreed with our tokenizer on a small server (claimed >100% cost where we measured +12% saving) — likely a different internal metric ([MCP-COMPRESSOR.md](MCP-COMPRESSOR.md)) |
